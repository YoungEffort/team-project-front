...
class Layouts extends Component {
   ...
   siderSubMenu = ({ key }) => {
      ...
      key !== '0' ? this.queryBreadcrumb(key) : this.setState({ breadcrumbData: [] })
   }
   // 递归查找跳转路由
   ...
   tapRemove = targetKey => {
      ...
      if (targetKey === tapActiveKey) {
         ...
         tapData[activeIndex-1].key !== '0' ? this.queryBreadcrumb(tapData[activeIndex-1].key) : this.setState({ breadcrumbData: [] })
         this.setState({
            isClickMenu: true,
   ...
   // 缓存菜单信息
   saveMenuData  = () => {
      let { selectedKeys, openKeys, tapData, tapActiveKey, breadcrumbData, pageTitle  } = this.state
      let menuData = JSON.parse(window.sessionStorage.getItem('menuData'))
      menuData = {
         ...menuData,
         selectedKeys,
